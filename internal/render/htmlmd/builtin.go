...
// BuiltinConverter is the legacy in-tree HTML→Markdown converter.
// Pure-Go, dependency-light (only golang.org/x/net/html), conservative
// in what it transforms. Handles the common set: headings, paragraphs,
// lists, blockquotes, code, links, images, line breaks. Skips chrome
// (script, style, nav, footer, aside, form, iframe, noscript).
//
// Kept as a fallback when the Kaufmann library can't be vendored, and
// as a regression baseline (the original output shape across two
// years of fetches still matches its current output).
type BuiltinConverter struct{}

func (*BuiltinConverter) Convert(htmlFragment string) string {
